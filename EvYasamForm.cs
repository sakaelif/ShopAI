...
namespace ShopAI
{
    public partial class EvYasamForm : Form
    {
        ...
        private void EvYasamForm_Load(object sender, EventArgs e)
        {
        }

        private void EvYasamForm_Activated(object sender, EventArgs e)
        {
            EvVeYasamUrunleriniYukle();
            lblEvVeYasamSepetAdet.Text = sepetServis.SepetToplamCesit().ToString();
        }

        private void EvVeYasamUrunleriniYukle()
        {
            flpEvYasamUrunler.Controls.Clear();

            evVeYasamUrunler = urunServis.EvVeYasamUrunleriListele();

            for (int i = 0; i < evVeYasamUrunler.Count; i++)
            {
                Panel urunKarti = EvVeYasamUrunKartiOlustur(evVeYasamUrunler[i]);
                flpEvYasamUrunler.Controls.Add(urunKarti);
            }
        }

        private Panel EvVeYasamUrunKartiOlustur(Urunler urun)
        {
        ...
        private void SepeteEkle(Urunler urun, int secilenAdet)
        {
            ...
            yeniSepetUrun.KategoriAdi = "Ev Yaşam";
        ...
        private void EvYasamForm_Load_1(object sender, EventArgs e)
        {

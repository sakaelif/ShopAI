...
namespace ShopAI
{
    public partial class ElektronikForm : Form
    {
        ...
        private List<Urunler> elektronikUrunler;
        ...
        {
            ElektronikUrunleriniYukle();
            lblElektronikSepetAdet.Text = sepetServis.SepetToplamCesit().ToString();
        }

        private void ElektronikForm_Activated(object sender, EventArgs e)
        {
            ElektronikUrunleriniYukle();
            lblElektronikSepetAdet.Text = sepetServis.SepetToplamCesit().ToString();
        }

        private void ElektronikUrunleriniYukle()
        {
            flpEvVeYasamUrunler.Controls.Clear();

            elektronikUrunler = urunServis.ElektronikUrunleriListele();

            for (int i = 0; i < elektronikUrunler.Count; i++)
            {
                Panel urunKarti = ElektronikUrunKartiOlustur(elektronikUrunler[i]);
                flpEvVeYasamUrunler.Controls.Add(urunKarti);
            }
        }

        private Panel ElektronikUrunKartiOlustur(Urunler urun)
        {
        ...
        private void SepeteEkle(Urunler urun, int secilenAdet)
        {
            ...
            yeniSepetUrun.KategoriAdi = "Elektronik";
        ...
        private void btnKategoriSecenegi_Click_1(object sender, EventArgs e)
        {
            this.Hide();
        ...
        private void btnAsistanaBaglan_Click_1(object sender, EventArgs e)
        {
            this.Hide();

            AIAsistanForm form = new AIAsistanForm();

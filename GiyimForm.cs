...
namespace ShopAI
{
    public partial class GiyimForm : Form
    {
        ...
        private List<Urunler> giyimUrunler;
        ...
        {
            GiyimUrunleriniYukle();
            lblGiyimSepetAdet.Text = sepetServis.SepetToplamCesit().ToString();
        }

        private void GiyimForm_Activated(object sender, EventArgs e)
        {
            GiyimUrunleriniYukle();
            lblGiyimSepetAdet.Text = sepetServis.SepetToplamCesit().ToString();
        }

        private void GiyimUrunleriniYukle()
        {
            flpGiyimUrunler.Controls.Clear();

            giyimUrunler = urunServis.GiyimUrunleriListele();

            for (int i = 0; i < giyimUrunler.Count; i++)
            {
                Panel urunKarti = GiyimUrunKartiOlustur(giyimUrunler[i]);
                flpGiyimUrunler.Controls.Add(urunKarti);
            }
        }

        private Panel GiyimUrunKartiOlustur(Urunler urun)
        {
        ...
        private void SepeteEkle(Urunler urun, int secilenAdet)
        {
            ...
            yeniSepetUrun.KategoriAdi = "Giyim";
